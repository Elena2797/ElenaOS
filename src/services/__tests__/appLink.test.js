...
test('sin conectar no pide nada privado al servidor', async () => {
  let calls = 0;
  const c = createAppLinkClient({ base: 'x', apiKey: 'k', storage: memoryStorage(), fetchImpl: async () => { calls++; return reply(200, {}); } });
...
test('el código correcto deja la app conectada y manda el token', async () => {
  ...
  const c = createAppLinkClient({
    base: 'https://api', apiKey: 'k', storage,
...
test('si el servidor rechaza el token, la app lo olvida', async () => {
  ...
  const c = createAppLinkClient({ base: 'x', apiKey: 'k', storage, fetchImpl: async () => reply(401, { ok: false }) });
...
test('un fallo de red no rompe la app ni la desconecta', async () => {
  ...
  const c = createAppLinkClient({ base: 'x', apiKey: 'k', storage, fetchImpl: async () => { throw new Error('offline'); } });
