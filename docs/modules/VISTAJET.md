...
## Specialist de Isabel: vencimiento de pasaporte (nuevo, 2026-08-06)
Segundo vertical slice de Isabel construido sobre el patrón formalizado en [core/DOMAIN_SPECIALISTS.md](../core/DOMAIN_SPECIALISTS.md) (el primero fue Salud/sueño). Sin tabla nueva — reutiliza `vj_state.passport_exp` (ya existía, solo se leía/mostraba en frontend) y la tabla genérica `interventions`.
...
- Sin UI propia — conversación/MCP únicamente, igual que el specialist de sueño. El modal manual de `openVjState()` sigue funcionando exactamente igual, sin cambios.
- Tests: `isabel-api/src/__tests__/vistajet.test.js` (lógica pura: riesgo, decisión, parser de fechas) + `vistajet.orchestration.test.js` (fake-db en memoria, 9 escenarios incluida condición de carrera).
...
- **HOTO readiness proactivo**: la lógica ya existe y es buena (`services/readiness.js`), pero es 100% client-side. Portarla a un specialist invocable por Isabel es más trabajo — candidato para después, no descartado.
