...
12. **ON/OFF es el ciclo laboral de la usuaria, y es CONTEXTO — nunca evidencia, nunca estado del avión.** `life_context.mode` significa exclusivamente: `ON` = Estefanía está en sus días de trabajo/rotación; `OFF` = está en sus días libres. **No** significa "hay avión asignado", "hay HOTO activo", ni "rotación operacional en curso" — son estados distintos y no se sincronizan causalmente con `vj_state.aircraft`/`vj_state.status`. Casos reales que lo demuestran: se puede estar ON sin avión asignado todavía; se puede cambiar de avión A→B sin dejar de estar ON; se puede entregar un avión y seguir ON esperando el siguiente. **Cómo se usa:** como señal de contexto del priority engine, solo para desempatar entre dominios que ya tienen el mismo `attention_mode` no-`clear` (ON da preferencia ligera a VistaJet; OFF, al resto). **Cómo NO se usa:** nunca para que un dominio `clear` deje de serlo, nunca para saltarse el rango de `attention_mode` (un `urgent` en cualquier dominio siempre gana a un `maintain` en VistaJet, sea cual sea el modo), nunca como evidencia citable. La prioridad sigue saliendo de evidencia real; ON/OFF solo decide el orden cuando la evidencia ya empata. Implementado en `globalContext.js` (`life_mode`, expuesto pero fuera de `classifyAttention`) y en el system prompt de `now.js`.
