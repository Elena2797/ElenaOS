...
- **Pre-HOTO verificado (2026-09-26, tarde):** Railway `SUCCESS` en el commit `7b0ede7` y `/mcp/http` lista `hoto_pre_*` en producción; ida y vuelta real (HOTO del 9H-VCF exportado a PDF → importado con notas de prueba → contraste → migrar → limpieza) OK. **Desplegado** (`isabel-api` `d84799a`, `life-os-app` `69d4c81`, tarjeta "Antes del HOTO" en la app): contraste por *mejor* coincidencia (`similarity`/`bestMatch`, `jump seat`=`jumpseat`, mínimo 2 palabras) y **entregar avión descarta las notas pre-HOTO** (`pre_hoto_notes_discarded`), 1053/1053. **Sin verificar:** que Isabel ve las tools tras `openclaw mcp reload` (lo lanza ella) y su comportamiento con las reglas Outlook/Calendar; el pre-HOTO no tiene pantalla (falta diseño + política RLS de `vj_pre_hoto_notes`).
