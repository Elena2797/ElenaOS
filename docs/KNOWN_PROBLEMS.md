...
### `vj_state` acumuló 3 filas en producción — RESUELTO (D14), acción pendiente
...
**Pendiente de ejecución manual:** `vj_state_singleton_migration_v1.sql` (`CREATE UNIQUE INDEX ON vj_state ((true))`) — sin este índice, la reconciliación de hoy es puntual, no una garantía estructural; si algún script vuelve a insertar sin comprobar, el mismo bug podría repetirse. No hay mecanismo programático de DDL en este proyecto (mismo motivo que `hoto_migration_v4.sql` sigue pendiente).
