...
### Gateway antiguo conservado como rollback con cron de sueño duplicado — RESUELTO 2026-08-08
Telegram sigue deshabilitado y ningún runtime productivo depende del Gateway antiguo. Con autorización administrativa explícita de la usuaria se deshabilitó únicamente su copia de `sleep-check-0800-madrid` mediante el RPC WebSocket oficial `cron.update` por loopback. El job y el Gateway se conservan para rollback; no se tocó SQLite. Verificación final: copia antigua `enabled:false`; Gateway nuevo conserva `proactive-tick-15m` y la única copia activa de `sleep-check-0800-madrid`.
