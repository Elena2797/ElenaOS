...
### 2. API key de isabel-api con fallback hardcodeado, y expuesta en el bundle del cliente — RESUELTO 2026-09-22
**Arreglo** (D68): `/v1` ya no depende de esa llave. La app entra con su token de app (`x-app-token`) y los PDF con un ticket de 30 minutos atado a ese PDF; los servidores, con la llave privada del Gateway o `API_KEY`, que ya no tiene valor por defecto (`isabel-api/src/core/access.js`, `2fcdb0b`). El bundle de la app no lleva ninguna llave (`life-os-app` `6c726b3`). Rotada por ella el 2026-09-22 (~15:35Z) con `cerrar-clave-publica.ps1`; verificado a las 15:37Z: `isabel-api-2026` da 401 en `/v1`, en `/mcp/http` y en `/oauth/*/start`; los dos ticks usan `ISABEL_MCP_KEY` (el de recordatorios responde 200 con ella) e Isabel sigue con 48 tools. Ella confirmó que la app carga en el móvil con su token. `API_KEY` nueva solo vive en Railway (hace falta para reconectar Instagram o Google por `?api_key=`).
...
**Después:** el resto de `/v1` y de las tools MCP públicas siguió con la llave del riesgo #2 hasta D68; cerrado al rotar `API_KEY` el mismo día.
